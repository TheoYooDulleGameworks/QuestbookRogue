...
public class JustDiceSlot : DiceSlot, IPointerDownHandler, IPointerUpHandler, IPointerClickHandler
{

    [Header("Conditions")]
    [SerializeField] private List<DiceType> diceTypes = new List<DiceType>();
    [SerializeField] private List<int> justConditionValues;
    ...
    {
        JustSlotSO justSlotData = slotData as JustSlotSO;

        if (justSlotData != null)
        {
            diceTypes = justSlotData.requestDiceTypes;
            justConditionValues = justSlotData.justDiceValues;
            defaultSprite = justSlotData.defaultSlotSprite;
            checkingSprite = justSlotData.checkingSlotSprite;
            succeedSprite = justSlotData.succeedSlotSprite;
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        if (collision.CompareTag("DiceValueCheck"))
        {
            ...
            if (rollDice.IsSameValue(diceTypes, justConditionValues))
            {
